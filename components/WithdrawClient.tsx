...
export default function WithdrawClient({ address, balance, earned }: WithdrawClientProps) {
  ...
  const totalBalance = balance + earned;
  ...
  const handleWithdraw = async () => {
    ...
    try {
      const { depositToYield } = await import("@/lib/lending");
      // In production this would call wallet.lending().withdraw()
      setStatus("success");
    } catch {
      setStatus("error");
    }
    ...
    <div className="max-w-md mx-auto mt-12 space-y-6">
      ...
      <div className="flex justify-between items-center">
        <div className="flex items-center gap-3">
          <Link href="/dashboard">
            <button className="w-9 h-9 rounded-xl bg-slate-800 hover:bg-slate-700 flex items-center justify-center transition-all active:scale-95 cursor-pointer">
      ...
      {/* Balance Card */}
      <div className="rounded-2xl p-6 border border-slate-800 bg-slate-900/80 space-y-4">
        ...
        <div className="flex justify-between text-xs">
          ...
          <span className="text-slate-400 font-mono">${balance.toFixed(2)}</span>
        </div>
        <div className="flex justify-between text-xs">
          <span className="text-slate-500">Yield earned</span>
          <span className="text-emerald-400 font-mono">+${earned.toFixed(7)}</span>
        </div>
      </div>

      {/* Success State */}
      {status === "success" && (
        <div className="rounded-2xl p-6 border border-emerald-500/20 bg-emerald-500/5 text-center space-y-3">
          ...
          <Link href="/dashboard">
            <button className="mt-3 px-6 py-3 rounded-xl bg-slate-800 hover:bg-slate-700 text-white text-sm font-semibold transition-all cursor-pointer">
              ← Back to Dashboard
            </button>
          </Link>
        </div>
      ...
      {status !== "success" && (
        <>
          <div className="space-y-2">
            ...
            <div className="relative">
              <input
                ...
                className="w-full bg-slate-900 border border-slate-700 rounded-xl px-4 py-4 pr-20 text-xl font-mono text-white placeholder-slate-600 focus:outline-none focus:border-emerald-500/50 focus:ring-1 focus:ring-emerald-500/20 transition-all disabled:opacity-50"
              />
              <button
                onClick={handleMax}
                disabled={status === "processing"}
          ...
          <div className="space-y-2">
            ...
            <div className="px-4 py-3 bg-slate-900 border border-slate-800 rounded-xl flex justify-between items-center">
